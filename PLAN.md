...
5. それらに**CMS**が実装されている（管理画面 + Firestoreで全文言を管理）
6. 会員登録画面 → DB連携
7. サイトログインに**Google連携**
...
- CMS統合: **Edible-Forest 側で新規実装**（外部microCMS依存は廃止）
...
- CMS: **自作CMS**（管理画面 + Firestore + TipTap）
- ポート: 3000

## 自作CMS 実スキーマ

- `cmsPages`: `pageId`(top/concept...), `heroTitle`, `heroCaption`, `body`, `conceptTag`, `conceptLinkLabel`, `slides[]` → **公開ページ文言**
- `ensembles`: アンサンブルコンテンツ
- `spots`: 宿泊拠点コンテンツ
- `reports`: `title`, `category`, `date`, `image`, `body`
- アクセス: `src/lib/cms.ts`（`getPage` / `getEnsembles` / `getSpots` / `getEnsemble` / `getReports` / `getReport`）。
...
`ensembles/{id}` … アンサンブル。**公開表示もFirestoreを正とする**
`spots/{id}` … 宿泊拠点。**公開表示もFirestoreを正とする**
...
- **S0 基盤（完了）**: `lib/access.ts` / `lib/cms.ts` / env / PLAN.md
- **S1 公開ページCMS化**: トップ/concept/ensembles/spots/reports を Firestore 駆動に。仮素材フォールバック維持。
