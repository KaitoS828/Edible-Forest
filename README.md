...
- **管理者向け画像アップロード**: 各adminフォームの画像欄でURL入力に加えファイル添付が可能（`src/components/admin/ImageUpload.tsx`）。保存は `POST /api/admin/upload`（admin SDK経由でStorageルールに依存せず、未認証は403）。**画像のみ・4MBまで**（Vercelのリクエストボディ上限4.5MB以下）。
