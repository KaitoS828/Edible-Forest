...
export default function NewsForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(mode === "new" ? "/api/admin/news" : `/api/admin/news/${newsId}`, {
  ...
  async function handleDelete() {
    if (!newsId || !window.confirm("このニュースを削除しますか？")) return;
    setDeleting(true);
    setError(null);
    ...
    <form onSubmit={handleSubmit} className="max-w-[760px] space-y-5">
      ...
      <Section title="基本情報">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <Field label="ラベル">
            <input className={inputClass} style={inputStyle} value={form.label} onChange={(e) => set("label", e.target.value)} placeholder="NEW! / INFO など" />
          </Field>
        </div>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Field label="カテゴリ">
            <input className={inputClass} style={inputStyle} value={form.category} onChange={(e) => set("category", e.target.value)} placeholder="ニュース" />
          </Field>
          <ImageUpload label="カバー画像" value={form.imageUrl} onChange={(url) => set("imageUrl", url)} />
        </div>
        <Field label="リンク先">
          <input className={inputClass} style={inputStyle} value={form.href} onChange={(e) => set("href", e.target.value)} placeholder="/reports/example または https://..." />
        </Field>
      </Section>

      <Section title="本文">
        <RichTextEditor content={form.body} onChange={(html) => set("body", html)} placeholder="ニュース本文を入力してください" />
      </Section>
      ...
      <div className="flex flex-col gap-3 border-t pt-5 sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: "#DCE3EA" }}>
        ...
        <div className="flex gap-3">
          <a href="/admin/news" className="rounded-md border px-4 py-2 text-sm font-medium" style={{ borderColor: "#CBD5E1", color: "#334155", backgroundColor: "#FFFFFF" }}>戻る</a>
