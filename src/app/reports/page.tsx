...
function ReportCard({ report }: { report: typeof REPORTS[0] }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <p className="text-xs mb-2" style={{ color: "#1A2B1E" }}>{report.date}</p>
        <div className="flex flex-wrap gap-1.5 mb-3">
          {report.tags.map((tag) => {
            const c = colorMap[tag] ?? "#4A7C59";
            return (
              <span key={tag} className="text-xs px-2.5 py-0.5 rounded-full font-medium" style={{ backgroundColor: c + "20", color: c }}>
                {tag}
              </span>
            );
          })}
        </div>
        <h4 className="text-base font-medium leading-snug line-clamp-3 mb-3" style={{ color: "#2C3A2E" }}>{report.title}</h4>
        <span className="text-sm font-medium" style={{ color: "#C4712E" }}>詳細へ →</span>
      </div>
    </a>
  );
}
        ...
        <div className="max-w-[1200px] mx-auto px-5 lg:px-10 py-12 md:py-16">
          ...
          {filtered.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-24" style={{ color: "#1A2B1E" }}>
              <TreeIcon size={48} />
              <p className="mt-4 text-base">該当する記事がありません</p>
            </div>
          )}
        </div>
