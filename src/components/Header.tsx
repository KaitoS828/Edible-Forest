  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b" style={{ backgroundColor: "rgba(255,255,255,0.95)", backdropFilter: "blur(8px)", borderColor: "rgba(0,95,2,0.15)" }}>
      <div className="max-w-[1200px] mx-auto px-5 lg:px-10 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm" style={{ color: "#1A2B1E" }}>
          {NAV.map((n) => (
            <a key={n.href} href={n.href} className="hover:text-[#3C6B4F] transition-colors whitespace-nowrap">
              {n.label}
            </a>
          ))}
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  <a
                    ...
                    className="px-4 py-2 rounded-full text-sm font-medium border transition-colors hover:bg-[#3C6B4F] hover:text-white hover:border-[#3C6B4F]"
      ...
      {open && (
        <div className="md:hidden border-t px-5 py-5 flex flex-col gap-4 text-sm" style={{ backgroundColor: "#FFFFFF", borderColor: "rgba(0,95,2,0.15)", color: "#1A2B1E" }}>
          {NAV.map((n) => (
            <a key={n.href} href={n.href} onClick={() => setOpen(false)} className="hover:text-[#3C6B4F] transition-colors">{n.label}</a>
          ))}
          <div className="flex gap-2 pt-2">
            {user ? (
              <>
                <a href="/member" onClick={() => setOpen(false)} className="flex-1 text-center px-4 py-2.5 rounded-full font-medium border" style={{ borderColor: "#3C6B4F", color: "#1A2B1E" }}>マイページ</a>
                <button onClick={signOut} className="px-4 py-2.5 rounded-full text-sm border" style={{ borderColor: "rgba(0,95,2,0.15)", color: "#1A2B1E" }}>ログアウト</button>
              </>
            ) : (
              <>
